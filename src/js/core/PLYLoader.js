		...
		function binaryReadElement( at, properties ) {
			...
			for ( let i = 0; i < properties.length; i ++ ) {
				...
				if ( property.type === 'list' ) {
					...
					const n = property.countReader.f( at + read );
					read += property.countReader.size;

					for ( let j = 0; j < n; j ++ ) {

						list.push( valueReader.f( at + read ) );
					...
					element[ property.name ] = valueReader.f( at + read );
					read += valueReader.size;
		...
		function setPropertyBinaryReaders( properties, body, little_endian ) {
			...
			function getBinaryReader( dataview, type, little_endian ) {
				...
				switch ( type ) {
					...
					case 'int8':	case 'char':	return { f: ( at ) => { return dataview.getInt8( at ) }, size: 1 };
					case 'uint8':	case 'uchar':	return { f: ( at ) => { return dataview.getUint8( at ) }, size: 1 };
					case 'int16':	case 'short':	return { f: ( at ) => { return dataview.getInt16( at, little_endian ) }, size: 2 };
					case 'uint16':	case 'ushort':	return { f: ( at ) => { return dataview.getUint16( at, little_endian ) }, size: 2 };
					case 'int32':	case 'int':		return { f: ( at ) => { return dataview.getInt32( at, little_endian ) }, size: 4 };
					case 'uint32':	case 'uint':	return { f: ( at ) => { return dataview.getUint32( at, little_endian ) }, size: 4 };
					case 'float32': case 'float':	return { f: ( at ) => { return dataview.getFloat32( at, little_endian ) }, size: 4 };
					case 'float64': case 'double':	return { f: ( at ) => { return dataview.getFloat64( at, little_endian ) }, size: 8 };
