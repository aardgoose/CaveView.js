import { materialColor, abs, distance, float, fwidth, mix, smoothstep, texture, varying, vec3, vec4, reference, positionLocal } from '../Three.js';

export const cursorColor = ( ctx, delta ) => {
    ...
    const cursorWidth = reference( 'cursorWidth', 'float', materials );

    const aDelta = abs( delta );
    const ss = smoothstep( 0.0, cursorWidth, cursorWidth.sub( aDelta ) );

    return aDelta.lessThan( cursorWidth.mul( 0.05 ) ).select(
 ...
 export const getTerrainHeight = ( du, terrain, position ) => {
...
export const location = ( ctx, color ) => {
    ...
    return lu.accuracy.greaterThanEqual( 0 ).select(
